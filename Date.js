...
class Date extends React.Component {
  ...
  checkDate = (local) => {
    if (
      local !== null &&
      moment(local).isBefore(moment().format("MM-DD-YYYY"))
    ) {
      this.props.resetAll();
    }
  ...
  render() {
    return (
      <DateWrapper>
        <Calendar>
          <Day>{this.props.date.day}</Day>
          <My>
            <Month>{this.props.date.month}</Month>
            <div>{this.props.date.year}</div>
          </My>
        </Calendar>
        <div>{this.props.date.weekday}</div>
      </DateWrapper>
    );
  }
}
